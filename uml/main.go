...
func main() {
	if s, err := runParser(os.Args[1:]); err != nil {
		panic(err.Error())
	} else {
		if s != nil {
			fmt.Println("Pushing file change to client.")
			generate(os.Args[1], s.Packages, s.Edges)
			//json.NewEncoder(os.Stdout).Encode(s)
		}
	}
}
func runParser(dirNames []string) (clientstruct *ClientStruct, err error) {
	// TODO handle error
	clientstruct, _ = GetStructsDirName(dirNames)
...
func generate(name string, packages []Package, edges []Edge) {
	graph := gographviz.NewEscape()
	graph.SetDir(true)
	name=`"`+name+`"`
	graph.SetName(name)

	graph.AddAttr(name, "shape", `record`)
	graph.AddAttr(name,"rankdir","LR")
	packCreated := false
	for _, pack := range packages {
		...
		for _, file := range pack.Files {
			if len(file.Structs) != 0 {
				if !packCreated {
					graph.AddSubGraph(name, "cluster_"+pack.Name, map[string]string{
						"label": pack.Name,
					})
					packCreated = true
					println(pack.Name, len(file.Structs))
				}
				for _, str := range file.Structs {
					newStruct(str, "cluster_"+pack.Name, graph)
...
func newStruct(stru Struct, parent string, graph *gographviz.Escape) {
	builder := &strings.Builder{}
	fmt.Fprintf(builder, `"<%s>%s`, strings.ToLower(stru.Name), stru.Name)
	for _, field := range stru.Fields {
		fmt.Fprintf(builder, "|<%s>%s", field.Name, generateField(field))
	}
	fmt.Fprintf(builder, `"`)
	graph.AddNode(parent, stru.Name, map[string]string{
...
func generateField(field Field) string {
	...
	return fmt.Sprintf("%s %15s : %s", exported, field.Name, field.Type.Literal)
